...
install_language_files() {
  ...
  if [[ -d "${source_dir}/messages" ]]; then
    # Install JSON files (unified format)
    if ! cp "${source_dir}/messages"/*.json "${messages_target_dir}/" 2>/dev/null; then
...
download_language_files_remote() {
  ...
  for lang in "${available_languages[@]}"; do
    local lang_url="${GITHUB_BASE_URL}/messages/${lang}.json"
    local lang_file="${messages_target_dir}/${lang}.json"

    temp_file=$(mktemp -t "lang_${lang}.XXXXXX") || {
      warn "Failed to create temp file for ${lang}.json"
      failed=1
      continue
    }
    ...
    if ! download_with_retry "${lang_url}" "${temp_file}"; then
      warn "Failed to download ${lang}.json"
      rm -f "${temp_file}"
      failed=1
      continue
    fi
    ...
    if [[ ! -s "${temp_file}" ]]; then
      warn "Downloaded ${lang}.json is empty"
      rm -f "${temp_file}"
      failed=1
      continue
    fi
    ...
    if ! jq empty "${temp_file}" 2>/dev/null; then
      warn "${lang}.json has invalid format"
      rm -f "${temp_file}"
      failed=1
      continue
    fi
    ...
    mv "${temp_file}" "${lang_file}" || {
      warn "Failed to install ${lang}.json"
      rm -f "${temp_file}"
      failed=1
      continue
    }
  ...
  if [[ ! -f "${messages_target_dir}/en.json" ]]; then
    error "Failed to install default language file (en.json)"
    return 1
  fi
...
save_user_config() {
  local language="$1"
  local components="$2"
  local config_file="${TARGET_DIR}/statusline-config.json"
...
EOF
  ...
  if ! jq empty "${config_file}" 2>/dev/null; then
    error "Failed to create valid JSON config"
    return 1
  fi
